...
/* Ce programme permet de passer un fichier .xls en paramètre afin de convertir le fichier en fichier .csv.
*/
...
function convertXLStoCSV($infile)
{
	...
	foreach($loadedSheetNames as $sheetIndex => $loadedSheetName) {
		...
		$objWriter->save($chemin."/".$departement.'_'.$semestre.'_'.$date3[2].$date4[2].'_'.$loadedSheetName.'.csv');		
...
if($argc != 2){
	echo "Erreur : Parametre programme".PHP_EOL;
	exit(1);
}
convertXLStoCSV(("./".$argv[1]));
// echo "./".$argv[1];
