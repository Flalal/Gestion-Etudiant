...
// creation des ue et du semestre
    ...
    //pour avoir les attributs de chaques etudiants (nom,prenom,redoublement,...)
    for (var i=0; i<toutEtudiant.length;i++){
		//pour avoir la moyenne de l'etudiant x 
		for (var j=0; j<toutEtudiant[i].semestres.length;j++){
		 	var noteEtudiant=toutEtudiant[i].semestres[j].moyenneSem;
    ...
    root.innerHTML+="<tr class='etudiant' name=i'etudiant' ><td name='numero'>" + numero
        + "</td> <td name='nom'>" + nom
	...
	photo.innerHTML+="<div class='modal fade bs-example-modal-sm" + index + "' tabindex='-1' role='dialog' aria-labelledby='mySmallModalLabel'>"+ 
         ...
         "<ul> <li>"+test.getNom().toUpperCase() + " " + test.getPrenom()+"	/	Né le: " +test.getDateNaissance()+" </li><li>Diplome: " +test.getBac()+"</li>"+"<li>Redoublement: " +test.getRedoubler()+"</li></ul>"+
					"</div>"+
				" </div>"+
			" </div>";
			...
			"<thead>"+
			  "<tr>"+
				...
				"<th>Coef</th><td>"+semestre.getCoefficientSem()+" </td><td>"+coefue1+" </td><td>"+coefue2+"</td></tr>"+
				 ...
				 "<tr> <th>Classement</th>"+"<td id='etudintClassement"+index+"'></td>"+"<td id='classInfo"+index+"'></td>"+"<td id='classGene"+index+"'></td>"+"</tr>"+
...
function afficherClassement(){
	var tmp=document.getElementByName('etudintClassement');
	for(var i=0;i<tmp.length;i++)
		tmp[i].innerHTML=positionEtudiant;
}
